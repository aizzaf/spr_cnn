...
cam = cv2.VideoCapture('fps=1/campuran_101_154_181_133_fps=1.mp4')
...
while (cam.isOpened()):
	try:
		...
		if interpreter.get_tensor(output_details[2]['index']).max() < 0.8:
			cv2.putText(frame, 'Bintang belum dipelajari',(10,50), cv2.FONT_HERSHEY_TRIPLEX, 2, (255,255,255), thickness=1)
			print('bintang belum dipelajari')
		else:
			cv2.circle(frame, (int(np.around(x*width+width/2)),int(np.around(y*height+height/2))), 50, (255,255,255), thickness=1)
			cv2.putText(frame, 'ID=' + str(ID[label]) + ' RA=' + str(RA[label]) + 'deg DE=' + str(DE[label]) + 'deg ROLL=' + str(rotations[rotation]) + 'deg', (10,50), cv2.FONT_HERSHEY_TRIPLEX, 2, (255,255,255), thickness=1)
		...
		imageio.mimsave('prediction_mp4.gif', images)
